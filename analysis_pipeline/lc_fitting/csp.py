...
"""This module fits CSP light curves using sncosmo"""
...
def fit_data(out_path, model, rest_bands=None, **kwargs):
    ...
    out_table = create_empty_summary_table(csp.band_names)
    for input_table in csp.iter_sncosmo_input(bands=rest_bands, verbose=True):

        # Create a new, incomplete row for the table
        new_row = [input_table.meta['cid']]
        band_count = count_points_per_band(input_table['band'], csp.band_names)
