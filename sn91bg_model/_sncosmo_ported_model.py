...
def bilinear_interp(x1, x2, y1, y2, f, x, y):
    ...
    Args:
        ...
        f           (list): A list containing four values at
...
class SN91bgSource(sncosmo.Source):
    ...
    def __init__(self):
        ...
        # Creat bi-cubic spline for phase and wavelength using all 35 templates
        self._model_flux = np.full((len(self._stretch), len(self._color)), None)
        for i in range(len(self._stretch)):
            for j in range(len(self._color)):
                self._model_flux[i][j] = RectBivariateSpline(
                    self._phase,
                    self._wave,
                    self._flux_values[i][j], kx=3,
                    ky=3)
    ...
    def _flux(self, phase, wave):
        ...
        f = [self._model_flux[st1][c1](phase, wave),
             self._model_flux[st1][c2](phase, wave),
             self._model_flux[st2][c1](phase, wave),
             self._model_flux[st2][c2](phase, wave)]

        # Bilinear interpolation for stretch and color
        return A * bilinear_interp(self._stretch[st1], self._stretch[st2],
                                   self._color[c1], self._color[c2], f, x1, c)
