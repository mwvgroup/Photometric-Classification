...
def generate_lc(num,model):
	"""
	Generate a series of light curves with specified model and write to csv files
	...
	# randomly generate stretch and color in 2D normal distribution (covariance matrix included)
	x1 = np.zeros(num)
	c = np.zeros(num)
	for i in range(num):
		stretch, color = multivariate_normal([0.975,0.557], [[0.096**2,-0.0110263], [-0.0110263,0.175**2]])
	...
	lcs = [sncosmo.realize_lcs(obs[i], model, [params[i]])[0] for i in range(num)]
	...
	lc_dump = Table([cid, redshifts, peakmjd, x1, c], names=['cid', 'z', 't0', 'stretch', 'color'])
