...
def read_lc(number, DATA_DIR):
    ...
    DETECT=False
    meta_data={}
    for line in file:  # This for-loop find the header_start, data_start and data_end 
        if header==True:
            if line[:7]=='VARLIST':
                d_start1=h_start+1
                d_end1=d_start1
    ...
    SNcosmo_table=Table([table['MJD'],['sdss'+b for b in table['FLT']],flux,\
...
def write_lc(num, DATA_DIR, path):
    ...
    for i in range(num):
        ...
        cid.append(i+1)
        table.write(os.path.join(path, 'sn91bg_{:05d}.csv'.format(i+1)), overwrite=True)
    metadata=Table([cid, meta['z'], meta['t0'], meta['x0'], meta['st'], meta['c']],\
